...
export const LogEntry = forwardRef<HTMLDivElement, LogEntryProps>(
  ({ log, style, measureRef, ...rest }, ref) => {
    ...
    // Detect changes to the height of the component
    useLayoutEffect(() => {
      if (entryRef.current) {
        measureRef?.(entryRef.current);
      }
    }, [isExpanded, measureRef]); // Only update when expanded state changes
